...
void Initialize(const int window_width, const int window_height, const char* window_title) {
  ...
  entity_manager.CreateSingleton<event::EventManager>({});
  entity_manager.CreateSingleton<input::InputManager>(
      input::InputManager{(entity_manager.GetSingleton<event::EventManager>().get()), &window});
...
void Tick() {
  entity_manager.GetSingleton<input::InputManager>()->Update(window);
  ProcessInput();
...
void ProcessInput() {
  ...
  if (input_manager.IsKeyDown(input::Key::E)) {
  ...
  if (input_manager.IsKeyDown(input::Key::Q)) {
    entity_manager.GetSingleton<graphics::Camera>()->transform.Rotate(
        -100.0F * static_cast<float>(time::GetDeltaTime()), glm::vec3(0.0F, 1.0F, 0.0F));
  }
}
...
void FramebufferSizeCallback(GLFWwindow* window, const int width, const int height) {
  ui::RenderWindow::ResizeRenderTarget(width, height);
