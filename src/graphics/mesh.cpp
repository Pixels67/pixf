...
Mesh::Mesh(const std::vector<Vertex>& vertices, const std::vector<unsigned int>& indices)
    : index_count_(indices.size()), vertices_(vertices), indices_(indices) {
...
void Mesh::Render(const Material& material, const ShaderManager& shader_manager,
  ...
  shader_manager.SetUniform(material.GetShader(), "proj", proj);
  shader_manager.SetUniform(material.GetShader(), "view", camera.GetViewMatrix());
  shader_manager.SetUniform(material.GetShader(), "model", transform.GetMatrix());
